...
public class FractionSet {
    ...
    private double sum__;

    public void sumSet() {
        sum__ = 0;
        System.out.println("sum is " + sumTraversal(root));
    }

    private double sumTraversal(Node root) {
        double sum = 0;
        if (root != null) {
            sum += root.value.value();
            sumTraversal(root.left);
            sumTraversal(root.right);
        }
        sum__ += sum;
        return sum__;
    }
